...
ren "%TELA%.umd.js" "external-%TELA%.js" >> %PATH_BUILDER%\tmp\%TELA%.tmp
...
:WHILE
	...
	xcopy /y/f/c %UI_CAMINHO%%UI_BUNDLES%\%TELA%.umd.js.map %SERVER%%SERVER_ASSETS%\* >> %PATH_BUILDER%\tmp\%TELA%.tmp 2>&1
	xcopy /y/f/c %UI_CAMINHO%%PTBR%\* %SERVER%%SERVER_ASSETS%\i18n >> %PATH_BUILDER%\tmp\%TELA%.tmp 2>&1
	xcopy /E/H/C/I/Y/F %UI_CAMINHO%\projects\%TELA%\src\i18n\* %SERVER%%SERVER_ASSETS%\i18n\%TELA%\* >> %PATH_BUILDER%\tmp\%TELA%.tmp 2>&1

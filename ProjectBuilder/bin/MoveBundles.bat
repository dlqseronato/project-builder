...
ren "%TELA%.umd.js" "external-%TELA%.js" >> %PATH_BUILDER%\log\%TELA%.log
...
erase /q/f/s *%TELA%* >> %PATH_BUILDER%\log\%TELA%.log 2>&1
SET /a INDEX = 1
:WHILE
	call cmd /c %PATH_BUILDER%\bin\Logger %PATH_BUILDER% %TELA% "Move Bundles - Tentativa %INDEX%" 3
	xcopy /y/f/c %UI_CAMINHO%%UI_BUNDLES%\external-%TELA%.js %UI_CAMINHO%%UI_ASSETS% >> %PATH_BUILDER%\tmp\%TELA%.tmp 2>&1
	xcopy /y/f/c %UI_CAMINHO%%UI_BUNDLES%\%TELA%.umd.js.map %UI_CAMINHO%%UI_ASSETS% >> %PATH_BUILDER%\tmp\%TELA%.tmp 2>&1
